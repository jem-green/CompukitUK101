...
5 DIMD(4)
10 GOTO 9000
20 INPUT"DIFFICULTY";DF
50 SC=53196
55 DF=INT(ABS(DF)):IFDF>10THEN20
60 GOSUB8500:FORI=1TO16:FORJ=0TO47
80 POKE(SC+I*64+J),187:NEXTJ,I
...
200 IFN1+N=3OR(N=1ANDP2+D<2)OR(N=2ANDP2+D>15)THEN150
220 IF N<=2 THEN 360
...
860 FORP2=P4TOP4+DSTEPD1:IFP2+D1>15ORP2+D1<2THEN1240
...
960 PP=SC+P1+P2*64+1:IFPEEK(PP)=187ANDPEEK(PP+1)=187THEND(4)=1
970 PP=PP-2:IFP1<3THEN990
980 IFPEEK(PP)=187ANDPEEK(PP-1)=187THEND(3)=1
990 PP=SC+P1+P2*64:IFP2>13THEN1030
1010 IFPEEK(PP+64)=187ANDPEEK(PP+128)=187THEND(2)=1
1030 IFP2<4THEN1070
1040 IFPEEK(PP-64)=187ANDPEEK(PP-128)=187THEND(1)=1
...
1160 IFN>2THEN1190
1165 IFP2+D>=15ORP2+D<=2THEN1240
...
3180 GOSUB8500:PRINT"FLOWERS?":GOTO3220
3200 GOSUB8500:PRINT"WELL DONE YOU HAVE SURVIVED THE MAZE!!"
3210 PRINT"TRY AGAIN WITH HIGHER DIFFICULTY"
3220 PRINT:INPUT"ANOTHER GAME";Q$:IFLEFT$(Q$,1)="Y"THEN20
3230 END
7000 R=USR(0):Q=PEEK(531):P3=P1:P4=P2:IFQ=87THENP4=P2-1
7010 IFQ=88THENP4=P2+1
7020 IFQ=65THENP3=P1-1
7030 IFQ=68THENP3=P1+1
7110 IFPEEK(P3+P4*64+SC)<>32THENRETURN
...
8500 FORI=1TO16:PRINT:NEXT:RETURN
9000 GOSUB8500:PRINT"          MAZE!!!":PRINT"          -------"
9005 PRINT:PRINT
9010 INPUT"INSTRUCTIONS";Q$:IFLEFT$(Q$,1)="N"THEN20
9020 IFLEFT$(Q$,1)="Y"THEN9040
9030 PRINT"ANSWER Y OR N":GOTO 9010
9040 PRINT:PRINT"IN THIS GAME YOU HAVE TO GET THROUGH"
9050 PRINT"THE MAZE, WHICH WILL APPEAR, WITHOUT"
9060 PRINT"THE FIRE-BREATHING DRAGON (";CHR$(181);
9070 PRINT") GETTING YOU"
9080 PRINT:PRINT"TO MOVE, USE THE KEYS 'AROUND' KEY 'S'"
9090 PRINT"(NO DIAGONAL MOVES):"
9100 PRINT:PRINT"        W=UP"
9120 PRINT"A=LEFT, S=STAY PUT,D=RIGHT"
9130 PRINT"        X=DOWN":PRINT:PRINT"GOOD LUCK!"
9135 PRINT"(DELETE LINES 9000 ON TO RUN ON A 4K MACHINE)"
9140 PRINT:INPUT"DIFFICULTY (1=EASY TO 10=VERY HARD)";DF
9150 GOTO 50
OK
